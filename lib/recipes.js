...
/**
 * Compact, nutrition-aware recipe shape that goes into the AI Overview prompt.
 * Unlike {@link RecipeListItem} it carries deterministic per-serving macros so
 * the model can answer "light dinner / high-protein" intents without doing (or
 * inventing) any arithmetic.
 *
...
const HIGH_PROTEIN_MIN_GRAMS_PER_SERVING = 15;
const KETO_MAX_CARBS_PER_SERVING = 12;
const MACRO_DIETARY_VALUES = new Set(["high-protein", "keto"]);
          ...
          SELECT COALESCE(
            jsonb_agg(
              jsonb_build_object(
                ...
                'amount', ri_profile.amount,
                'unit', ri_profile.unit,
                'name', COALESCE(i_profile.name, ''),
                'calories', COALESCE(i_profile.calories, 0),
                'protein', COALESCE(i_profile.protein, 0),
                'carbs', COALESCE(i_profile.carbs, 0),
                'fat', COALESCE(i_profile.fat, 0),
                'nutritionBasis', COALESCE(i_profile.nutrition_basis, '${NUTRITION_BASIS_PER_100G}'),
                'unitWeights', COALESCE(i_profile.unit_weights, '{}'::jsonb),
 ...
 * @returns {string[]}
 */
const orderDietaryValues = (values) =>
  DIETARY_OPTIONS.map((option) => option.value).filter((value) =>
    values.has(value),
  );
...
/**
 * Macro-oriented diets are recipe-level properties. They depend on the full
 * recipe nutrition and explicit recipe tags, not on every ingredient carrying
 * the same macro label.
 *
 * @param {Recipe | null | undefined} recipe
 * @param {Map<string, Ingredient>} ingredientMap
 * @returns {Set<string>}
 */
const deriveRecipeMacroDietaryValues = (recipe, ingredientMap) => {
  const tags = getRecipeTagSet(recipe);

  if (!tags.has("high-protein") && !tags.has("keto")) {
    return new Set();
  }
  ...
  for (const recipeIngredient of ingredients) {
    ...
    const { nutrition, isUnconvertedIngredient } = getRecipeIngredientNutrition(
      recipeIngredient,
      ingredient,
    );
  ...
  const perServing = divideNutrition(roundNutrition(totalNutrition), servings);
  /** @type {Set<string>} */
  const macroDiets = new Set();

  if (
    tags.has("high-protein") &&
    perServing.protein >= HIGH_PROTEIN_MIN_GRAMS_PER_SERVING
  ) {
    macroDiets.add("high-protein");
  }

  if (tags.has("keto") && perServing.carbs <= KETO_MAX_CARBS_PER_SERVING) {
    macroDiets.add("keto");
  }
 ...
 */
const deriveRecipeDietaryProfileForRecipe = (recipe, ingredientMap) => {
  const baseProfile = deriveRecipeDietaryProfile(
    Array.isArray(recipe?.ingredients) ? recipe.ingredients : [],
    ingredientMap,
  );
  const diets = new Set(
    baseProfile.dietary.filter((diet) => !MACRO_DIETARY_VALUES.has(diet)),
  );

  for (const macroDiet of deriveRecipeMacroDietaryValues(
    recipe,
    ingredientMap,
  )) {
    diets.add(macroDiet);
  ...
  return {
    dietary: orderDietaryValues(diets),
    allergens: baseProfile.allergens,
    hasMissingIngredients: baseProfile.hasMissingIngredients,
  };
};
...
const matchesRecipeDietaryFilters = (
  ...
  return matchesDietaryProfileFilters(
    dietaryProfile ??
      deriveRecipeDietaryProfileForRecipe(recipe, ingredientMap),
    filters,
  );
};
...
const toPostgresDietaryMetadata = (metadata) => {
  ...
  for (const row of rows) {
    ...
    const typedRow =
      /** @type {{ ingredientId?: unknown, isMissing?: unknown, amount?: unknown, unit?: unknown, name?: unknown, calories?: unknown, protein?: unknown, carbs?: unknown, fat?: unknown, nutritionBasis?: unknown, unitWeights?: unknown, dietary?: unknown, commonAllergens?: unknown }} */ (
        row
      );
    ...
    recipeIngredients.push({
      ingredientId,
      amount:
        typeof typedRow.amount === "number"
          ? String(typedRow.amount)
          : toSafeString(typedRow.amount),
      unit: toSafeString(typedRow.unit),
    });
    ...
    ingredientMap.set(ingredientId, {
      ...
      category: "",
      nutrition: {
        calories: toPostgresNumber(typedRow.calories),
        protein: toPostgresNumber(typedRow.protein),
        carbs: toPostgresNumber(typedRow.carbs),
        fat: toPostgresNumber(typedRow.fat),
      },
      nutritionBasis:
        toSafeString(typedRow.nutritionBasis) || NUTRITION_BASIS_PER_100G,
      unitWeights: toUnitWeights(typedRow.unitWeights),
...
const getRecipeListFromPostgres = async (filters, sort) => {
  ...
  return rows.reduce((items, row) => {
    ...
    const dietaryProfile = deriveRecipeDietaryProfileForRecipe(
      recipe,
      ingredientMap,
    );
...
/**
 * Reads the whole catalog in one query (no WHERE, same shape that backs
 * {@link getRecipeFacetsFromPostgres}) and projects each recipe to the compact
 * nutrition-aware {@link OverviewCatalogRow} via the shared
 * {@link toOverviewCatalogRow} mapper. Malformed/unlistable rows are dropped so
 * the AI Overview prompt only ever sees real recipes.
 *
 * @returns {Promise<OverviewCatalogRow[]>}
 */
const getRecipeOverviewCatalogFromPostgres = async () => {
  const sql = await getSql();
  const { queryText, params } = createRecipeListQuery(
    normalizeRecipeListFilters({}),
    DEFAULT_RECIPE_SORT,
  ...
  const rows = /** @type {RecipeListRow[]} */ (
    await withNeonReadRetry(() => sql.query(queryText, params), {
      label: "Recipe overview catalog read",
    })
  );

  return rows.reduce((catalog, row) => {
    const { recipe, ingredientMap } = toRecipeListRecipeFromPostgres(row);
    const catalogRow = toOverviewCatalogRow(recipe, ingredientMap);
...
/**
 * Maps a raw {@link Recipe} into the compact {@link OverviewCatalogRow} used to
 * prompt the AI Overview. Reuses {@link toRecipeDetail} as the single authority
 * for per-serving nutrition and dietary/allergen derivation, then projects to
 * the small prompt-facing field set. Returns `null` for records that are not
 * listable (no non-empty id/title), matching the other mappers' contract so the
 * model never sees a malformed row.
 *
 * @param {Recipe | null | undefined} recipe
 * @param {Map<string, Ingredient>} ingredientMap
 * @returns {OverviewCatalogRow | null}
 */
const toOverviewCatalogRow = (recipe, ingredientMap = new Map()) => {
  const detail = toRecipeDetail(recipe, ingredientMap);

  if (
    !detail ||
    !isNonEmptyString(detail.id) ||
    !isNonEmptyString(detail.title)
  ) {
    return null;
  }
  ...
  return {
    ...
    title: detail.title,
    tags: Array.from(
      new Set(detail.tags.map((tag) => toSearchText(tag)).filter(Boolean)),
    ),
    difficulty: detail.difficulty,
    prepMinutes: parseDurationMinutes(detail.prepTime),
    cookMinutes: parseDurationMinutes(detail.cookTime),
    servings: detail.servings,
    dietary: detail.dietary,
    allergens: detail.allergens,
    nutritionComplete:
      detail.nutrition.missingIngredientIds.length === 0 &&
      detail.nutrition.unconvertedIngredientIds.length === 0,
    perServing: detail.nutrition.perServing,
  };
};
...
/** @type {(() => Promise<OverviewCatalogRow[]>) | null} */
let cachedRecipeOverviewCatalogFromPostgres = null;
...
const getCachedRecipeOverviewCatalogFromPostgres = async () => {
  if (!cachedRecipeOverviewCatalogFromPostgres) {
    const { unstable_cache: unstableCache } = await importNextCache();
    cachedRecipeOverviewCatalogFromPostgres = unstableCache(
      getRecipeOverviewCatalogFromPostgres,
      ["recipe-overview-catalog"],
      {
        revalidate: RECIPES_CACHE_REVALIDATE_SECONDS,
        tags: [RECIPES_CACHE_TAG],
      },
  ...
  return cachedRecipeOverviewCatalogFromPostgres;
};
...
/**
 * Loads the compact, nutrition-aware catalog the AI Overview feeds to the model.
 * Cached under the shared `recipes` tag (so it invalidates with every other
 * read) with the same incremental-cache fallback used elsewhere, keeping it
 * usable from jest/seed scripts that run outside Next's cache context.
 *
 * @returns {Promise<OverviewCatalogRow[]>}
 */
const getRecipeOverviewCatalog = async () => {
  const cachedRead = await getCachedRecipeOverviewCatalogFromPostgres();
