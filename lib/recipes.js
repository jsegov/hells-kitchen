...
const countRecipesWithOption = (
  ...
  for (const recipe of listableRecipes) {
    if (matchesRecipeListFilters(recipe, candidateFilters, ingredientMap)) {
      count += 1;
    }
  }
 ...
 * diet/allergen from the static option lists) each annotated with a drill-down
 * result count.
 *
...
const computeRecipeFacets = (recipes, ingredientMap, filters) => {
  ...
  return {
    ...
    diets: DIETARY_OPTIONS.map((option) => ({
      value: option.value,
      label: option.label,
      count: count("diets", option.value),
    })),
    allergens: ALLERGEN_OPTIONS.map((option) => ({
      value: option.value,
      label: option.label,
