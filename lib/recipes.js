...
const getRecipeSortValue = (recipe, sort) => {
  ...
  if (sort === "servings") {
    return toSafeNumber(recipe.servings);
  }
 ...
 */
const toRecipeListItem = (recipe, ingredientMap = new Map()) => {
  if (!recipe || typeof recipe !== "object") {
    return null;
  }
...
const toRecipeListItemFromPostgres = (row) => {
  ...
  const item = toRecipeListItem(recipe, ingredientMap);
...
const getRecipeListFromPostgres = async (filters, sort) => {
  ...
  return rows.reduce((items, row) => {
    ...
    if (!matchesRecipeDietaryFilters(recipe, filters, ingredientMap)) {
      return items;
    }
