  ...
  return {
    queryText: `
      SELECT
        ...
        r.difficulty,
        r.tags,
        COUNT(ri_count.ingredient_id) FILTER (
          WHERE ri_count.ingredient_id <> ''
        ) AS ingredient_count
      FROM recipes r
      LEFT JOIN recipe_ingredients ri_count ON ri_count.recipe_id = r.id
      ${whereClause}
      GROUP BY
        r.id,
        r.sort_order,
...
const getCachedRecipeListFromPostgres = async () => {
  if (!cachedRecipeListFromPostgres) {
    const { unstable_cache: unstableCache } = await import("next/cache.js");
